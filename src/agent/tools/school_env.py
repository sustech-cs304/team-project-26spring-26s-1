...
Priority for IDs/passwords:
  ...
  3. Environment variables (TIS_* / BB_* / SUSTECH_* / TIS_DEFAULT_*)
...
def resolve_bb_credentials(user_name: str | None, pwd: str | None) -> tuple[str, str]:
    """Tool args first, then Global Settings runtime CAS, then BB_* / SUSTECH_*."""
    ru, rp = _get_tis_runtime_pair()
    u = user_name or ru or os.getenv("BB_STUDENT_ID") or os.getenv("SUSTECH_STUDENT_ID")
    p = pwd or rp or os.getenv("BB_CAS_PASSWORD") or os.getenv("SUSTECH_CAS_PASSWORD")
    return u, p


def resolve_tis_credentials(user_name: str | None, pwd: str | None) -> tuple[str, str]:
    """Tool args first, then API runtime override, then TIS_*, then SUSTECH_*."""
    ru, rp = _get_tis_runtime_pair()
    u = user_name or ru or os.getenv("TIS_STUDENT_ID") or os.getenv("SUSTECH_STUDENT_ID")
    p = pwd or rp or os.getenv("TIS_CAS_PASSWORD") or os.getenv("SUSTECH_CAS_PASSWORD")
    return u, p
...
def missing_cas_message() -> dict:
    return {
        ...
        "message": (
            ...
            "or set SUSTECH_STUDENT_ID / SUSTECH_CAS_PASSWORD or BB_*/TIS_* in the environment."
        ),
    }
