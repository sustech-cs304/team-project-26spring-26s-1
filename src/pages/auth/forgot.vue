    ...
    <v-row justify="center">
      <v-col cols="12" sm="10" md="8" lg="6" xl="4">
        ...
        <v-card class="pa-4" color="grey-darken-4" rounded="lg">
          ...
          <v-form ref="formRef" @submit.prevent="onSubmit">
            <p class="text-body-2 text-grey mb-4 text-center">
                Enter your user account's verified email address and we will send you a verification code to reset your password.
            </p>

            <AuthTextField v-model="form.email" label="Email" placeholder="Enter your email address." icon="mdi-email-outline" :rules="emailRules">
            ...
            <AuthTextField v-model="form.verificationCode" label="Verification Code" placeholder="Enter verification code" icon="mdi-shield-key-outline" :rules="requiredRules"/>

            <AuthTextField v-model="form.newPassword" label="New Password" placeholder="At least 6 characters" icon="mdi-lock-outline" isPassword :rules="passwordRules"/>
            <AuthTextField v-model="form.confirmPassword" label="Confirm Password" placeholder="Re-enter your new password" icon="mdi-lock-check-outline" isPassword :rules="confirmPasswordRules"/>

            <v-alert v-if="errorMsg" type="error" variant="tonal" class="mb-4">
              {{ errorMsg }}
            </v-alert>

            <v-btn type="submit" block size="large" color="cyan-darken-2" class="text-none mb-3" :loading="submitLoading" :disabled="submitLoading">
                Reset Password
        ...
        <div class="d-flex align-center justify-center mt-6 text-grey">
          ...
          <span class="text-body-2">Reset link will be sent to your campus email</span>
        </div>
      </v-col>
    </v-row>
  </v-container>
