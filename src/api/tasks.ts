...
import type { Task, TaskForm } from '@/utils/tasks'
 ...
 * PATCH /tasks/{id}
 */
export function updateTask (id: number, data: Partial<TaskForm>): Promise<Task> {
...
export function deleteTask (id: number): Promise<{ message: string }> {
    return http.delete<{ message: string }>(`/tasks/${id}`)
}
 ...
 */
export function setTaskStatus (id: number, status: 'running' | 'paused'): Promise<Task> {
    return http.patch<Task>(`/tasks/${id}/status`, { status })
}
 ...
 * POST /tasks/{id}/run
 */
export function runTask (id: number): Promise<{ message: string }> {
    return http.post<{ message: string }>(`/tasks/${id}/run`)
}
